...
export const CourseDetails = () => {
    ...
    const {allcourses, calculateRating,calculateChapterTime,calculateNoOfLectures,
      calculateCourseDuration,currency} = useContext(AddContext)
              ...
              <div className='flex'>
                {[...Array(5)].map((_,i)=> (<img key={i} src={i < Math.floor(calculateRating(courseData)) ? 
                    ...
                    <p className='text-blue-500'>({courseData.courseRatings.length}
                      {courseData.courseRatings.length >1 ? 'ratings' : 'ratings'})</p>

                    <p> {courseData.enrolledStudents.length}
                  {courseData.enrolledStudents.length >1 ? 'students' : 'student'}</p>
          </div>

                        <p className='text-sm'>Course by <span className='text-blue-600'>Techie</span></p>
                    ...
                    <div className='py-20 text-sm md:text-default'>
                            <h3 className='text-xl fomt-semibold text-gray-800'>Course Description</h3>
